...
from .dependencies import (
    save_image_in_cognitive_search_vectorial_db,
    answer_question
)
...
@router.post("/load_document", response_description="Preprocesa y almacena la imagen que se le indique para sar usado posteriormente en la resolucion de preguntas")
async def load_document(
    ...
    redis_index_name = f"{user_id}_{session_id}_image"

    response = save_image_in_cognitive_search_vectorial_db(file.filename, redis_index_name)
...
async def get_response(input: UserMessageModel) -> AIResponseModel:
    ...
    redis_index_name = f"{input.user_id}_{input.session_id}_image"
